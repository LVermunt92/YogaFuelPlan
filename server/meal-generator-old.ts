...
export function generateWeeklyMealPlan(request: MealPlanRequest, user?: User): GeneratedMealPlan {
  ...
  if (cookingDays.length < eatingDays.length) {
    ...
    return generateMealPrepPlan(request, user, cookingDays, eatingDays, caloricAdjustment);
  }
  ...
  for (let day = 1; day <= 8; day++) {
    ...
    mealsToGenerate.forEach((mealType) => {
      ...
      if (day === 1 && mealCategory === 'dinner') {
        ...
        mondayDinnerMeal = selectedMeal; // Store for Tuesday lunch leftover
      } else if (day === 3 && mealCategory === 'lunch') {
        // Day 3: Tuesday lunch - leftover from Monday dinner (EXACT SAME MEAL)
        if (!mondayDinnerMeal) {
          throw new Error('Monday dinner meal not found for Tuesday lunch leftover');
        }
        selectedMeal = mondayDinnerMeal; // Use exact same meal as Monday dinner
        isLeftover = true;
        ...
        // Day 7: Saturday lunch - leftover from Friday dinner (EXACT SAME MEAL)
        if (!thursdayDinnerMeal) {
          throw new Error('Thursday dinner meal not found for Saturday lunch leftover');
        }
        selectedMeal = thursdayDinnerMeal; // Use exact same meal as Friday dinner (Thursday leftover)
        isLeftover = true;
...
function generateMealPrepPlan(
  ...
  // With 3 cooking days and 6 eating days, each cooking day produces 2 meals
  // Plan: 3 fresh cooking meals + 3 leftover meals = 6 total meals
  
  const selectedMeals: Array<{ meal: MealOption, isLeftover: boolean, day: number, mealType: 'lunch' | 'dinner' }> = [];
  
  // Cooking Day 1: Fresh lunch
  selectedMeals.push({
    meal: lunchOptions[0] || lunchOptions[Math.floor(Math.random() * lunchOptions.length)],
    isLeftover: false,
    day: cookingDays[0],
    mealType: 'lunch'
  });
  
  // Cooking Day 2: Fresh dinner  
  selectedMeals.push({
    meal: dinnerOptions[0] || dinnerOptions[Math.floor(Math.random() * dinnerOptions.length)],
    isLeftover: false,
    day: cookingDays[1],
    mealType: 'dinner'
  });
  
  // Cooking Day 3: Fresh lunch
  selectedMeals.push({
    meal: lunchOptions[1] || lunchOptions[Math.floor(Math.random() * lunchOptions.length)],
    isLeftover: false,
    day: cookingDays[2],
    mealType: 'lunch'
  });
  
  // Leftover meals (using the same recipes from cooking days)
  selectedMeals.push({
    meal: selectedMeals[0].meal, // Same as cooking day 1 lunch
    isLeftover: true,
    day: eatingDays[3], // 4th eating day
    mealType: 'dinner'
  });
  
  selectedMeals.push({
    meal: selectedMeals[1].meal, // Same as cooking day 2 dinner
    isLeftover: true,
    day: eatingDays[4], // 5th eating day
    mealType: 'lunch'
  });
  
  selectedMeals.push({
    meal: selectedMeals[2].meal, // Same as cooking day 3 lunch
    isLeftover: true,
    day: eatingDays[5], // 6th eating day
    mealType: 'dinner'
  });

  // Generate actual meal objects from the selected meals
  selectedMeals.forEach(({ meal, isLeftover, day, mealType }) => {
    // Adjust portion based on caloric goals and household size
    let portionMultiplier = caloricAdjustment;
    if (user?.householdSize && user.householdSize > 1) {
      portionMultiplier *= user.householdSize;
    ...
    const adjustedPortion = adjustMealPortion(meal.portion, portionMultiplier);
    const adjustedProtein = Math.round(meal.nutrition.protein * caloricAdjustment);
    const prepTimeForDay = isLeftover ? 5 : meal.nutrition.prepTime;

    const mealObj: InsertMeal = {
      mealPlanId: 0,
      day,
      mealType,
      foodDescription: isLeftover ? `${meal.name} (leftover)` : meal.name,
      portion: adjustedPortion,
      protein: adjustedProtein,
      prepTime: prepTimeForDay,
    };
