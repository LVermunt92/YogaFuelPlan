...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/fridge", async (req, res) => {
    try {
      ...
      const items = await storage.getFridgeItems(req.session.userId, includeUsed);
      res.json(items);
    } catch (error) {
  ...
  app.post("/api/fridge", async (req, res) => {
    try {
      ...
      const itemData = fridgeItemSchema.parse(req.body);
      const newItem = await storage.addFridgeItem({
        ...itemData,
        userId: req.session.userId,
        expirationDate: itemData.expirationDate ? new Date(itemData.expirationDate) : null,
        isUsed: false
      });
  ...
  app.get("/api/fridge/:id", async (req, res) => {
    try {
      if (!req.session?.userId) {
        return res.status(401).json({ message: "Not authenticated" });
      }
      ...
      const id = parseInt(req.params.id);
      const item = await storage.getFridgeItem(id, req.session.userId);
      ...
      res.status(500).json({ message: "Failed to fetch fridge item" });
    }
  });

  // Update fridge item
  app.patch("/api/fridge/:id", async (req, res) => {
    try {
      if (!req.session?.userId) {
        return res.status(401).json({ message: "Not authenticated" });
      }
      ...
      const item = await storage.updateFridgeItem(id, req.session.userId, updates);
      res.json(item);
    } catch (error) {
  ...
  app.delete("/api/fridge/:id", async (req, res) => {
    try {
      ...
      const id = parseInt(req.params.id);
      await storage.deleteFridgeItem(id, req.session.userId);
      res.status(204).send();
    } catch (error) {
  ...
  app.patch("/api/fridge/:id/use", async (req, res) => {
    try {
      if (!req.session?.userId) {
        return res.status(401).json({ message: "Not authenticated" });
      }
      ...
      const id = parseInt(req.params.id);
      await storage.markFridgeItemAsUsed(id, req.session.userId);
      res.status(204).send();
    } catch (error) {
      console.error("Error marking fridge item as used:", error);
      res.status(500).json({ message: "Failed to mark item as used" });
    }
  });

  // Search fridge items by ingredient name
  app.get("/api/fridge/search/:ingredient", async (req, res) => {
    try {
      if (!req.session?.userId) {
        return res.status(401).json({ message: "Not authenticated" });
      }
      ...
      const items = await storage.getFridgeItemsByIngredient(req.session.userId, ingredient);
      res.json(items);
    } catch (error) {
      console.error("Error searching fridge items:", error);
      res.status(500).json({ message: "Failed to search fridge items" });
    }
  });
