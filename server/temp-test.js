...
exports.ENHANCED_MEAL_DATABASE = void 0;
exports.getCompleteEnhancedMealDatabase = getCompleteEnhancedMealDatabase;
exports.getEnhancedMealsByCategory = getEnhancedMealsByCategory;
exports.filterEnhancedMealsByDietaryTags = filterEnhancedMealsByDietaryTags;
exports.getEnhancedMealsForCategoryAndDiet = getEnhancedMealsForCategoryAndDiet;
exports.generateEnhancedShoppingList = generateEnhancedShoppingList;
var ayurveda_seasonal_1 = require("./ayurveda-seasonal");
var viral_recipe_updater_1 = require("./viral-recipe-updater");
var recipe_expansion_1 = require("./recipe-expansion");
// Enhanced meal database focusing on whole foods and minimal processing
exports.ENHANCED_MEAL_DATABASE = [
...
function getCompleteEnhancedMealDatabase() {
    var viralRecipes = (0, viral_recipe_updater_1.getCurrentViralRecipes)();
    var additionalRecipes = (0, recipe_expansion_1.getAllAdditionalRecipes)();
    console.log("\uD83D\uDD25 Adding ".concat(viralRecipes.length, " viral recipes and ").concat(additionalRecipes.length, " additional recipes to database"));
    return __spreadArray(__spreadArray(__spreadArray([], exports.ENHANCED_MEAL_DATABASE, true), viralRecipes, true), additionalRecipes, true);
}
// Function to get meals from enhanced database filtered by dietary requirements
function getEnhancedMealsByCategory(category) {
    var completeDatabase = getCompleteEnhancedMealDatabase();
    return completeDatabase.filter(function (meal) { return meal.category === category; });
}
function filterEnhancedMealsByDietaryTags(meals, dietaryTags) {
    if (dietaryTags.length === 0)
        return meals;
    return meals.filter(function (meal) {
        // Handle critical dietary restrictions that must be enforced
        var criticalTags = ['vegetarian', 'vegan', 'gluten-free', 'lactose-free', 'dairy-free'];
        var userCriticalTags = dietaryTags.filter(function (tag) { return criticalTags.includes(tag); });
        // All critical dietary tags must be satisfied
        if (userCriticalTags.length > 0) {
            var criticalSatisfied = userCriticalTags.every(function (tag) { return meal.tags.includes(tag); });
            if (!criticalSatisfied)
                return false;
        }
        // For preference tags (high-protein, ayurvedic, etc.), be more flexible
        var preferenceTags = dietaryTags.filter(function (tag) { return !criticalTags.includes(tag); });
        if (preferenceTags.length === 0)
            return true; // Only critical tags were specified
        // At least 50% of preference tags should match (allows for some variety)
        var matchingPreferenceTags = preferenceTags.filter(function (tag) { return meal.tags.includes(tag); });
        return matchingPreferenceTags.length >= Math.ceil(preferenceTags.length * 0.5);
    });
}
function getEnhancedMealsForCategoryAndDiet(category, dietaryTags) {
    if (dietaryTags === void 0) { dietaryTags = []; }
    var categoryMeals = getEnhancedMealsByCategory(category);
    var filteredMeals = filterEnhancedMealsByDietaryTags(categoryMeals, dietaryTags);
    // Apply seasonal adaptations for ayurvedic meals
    if (dietaryTags.includes('ayurvedic') && filteredMeals.length > 0) {
        var currentSeason_1 = (0, ayurveda_seasonal_1.getCurrentAyurvedicSeason)(new Date(), 'europe');
        var seasonalGuidance_1 = (0, ayurveda_seasonal_1.getCurrentSeasonalGuidance)(new Date(), 'europe');
        ...
        if (currentSeason_1 === 'grishma') {
            var originalCount = filteredMeals.length;
            filteredMeals = filteredMeals.filter(function (meal) {
                if (!meal.tags.includes('ayurvedic'))
                    return true; // Keep non-ayurvedic meals
                var hasWarmingTags = meal.tags.includes('warming');
                var hasHeatingSpices = meal.ingredients.some(function (ingredient) {
                    return ingredient.toLowerCase().includes('ginger') && !ingredient.toLowerCase().includes('fresh ginger') ||
                ...
                if (hasWarmingTags || hasHeatingSpices) {
                    console.log("\uD83D\uDEAB Summer exclusion: ".concat(meal.name, " removed (inappropriate warming characteristics for grishma season)"));
            ...
            if (originalCount !== filteredMeals.length) {
                console.log("Summer filter: ".concat(originalCount, " \u2192 ").concat(filteredMeals.length, " ayurvedic meals (excluded warming recipes)"));
            }
        }
        filteredMeals = filteredMeals.map(function (meal) {
            if (meal.tags.includes('ayurvedic')) {
                var adaptedMeal = (0, ayurveda_seasonal_1.adaptRecipeForSeason)(meal, currentSeason_1);
                // Add seasonal context to the portion for display, keep original name for recipe lookup
                adaptedMeal.portion = "".concat(meal.portion, " (").concat(seasonalGuidance_1.season, " season)");
                return adaptedMeal;
            }
            return meal;
        });
    ...
    if (filteredMeals.length === 0) {
        console.log("No ".concat(category, " meals found for dietary tags: ").concat(dietaryTags.join(', '), ". Attempting smart fallback."));
        // Define critical dietary restrictions that should never be violated
        var criticalTags_1 = ['vegetarian', 'vegan', 'kosher', 'halal'];
        var criticalUserTags = dietaryTags.filter(function (tag) { return criticalTags_1.includes(tag); });
        // If user has critical dietary restrictions, only respect those in fallback
        if (criticalUserTags.length > 0) {
            var criticalFilteredMeals = filterEnhancedMealsByDietaryTags(categoryMeals, criticalUserTags);
            if (criticalFilteredMeals.length > 0) {
                console.log("Fallback: Found ".concat(criticalFilteredMeals.length, " ").concat(category, " meals respecting critical dietary restrictions: ").concat(criticalUserTags.join(', ')));
        ...
        console.error("CRITICAL: No ".concat(category, " meals found that respect dietary restrictions. This should not happen."));
        return [];
    }
    return filteredMeals;
}
function generateEnhancedShoppingList(meals, language, dietaryTags) {
    if (language === void 0) { language = 'en'; }
    if (dietaryTags === void 0) { dietaryTags = []; }
    var ingredientAmounts = new Map();
    // Parse actual recipe amounts from meal instructions
    meals.forEach(function (meal) {
        // Strip any leftover suffix and portion scaling for meal matching
        var cleanMealName = meal.foodDescription
            .replace(/ \(leftover\)$/, '')
            .replace(/ \(2\.0x.*?\)$/, '')
            .replace(/ \- batch cook$/, '')
            .trim();
        var mealOption = exports.ENHANCED_MEAL_DATABASE.find(function (m) { return m.name === cleanMealName; });
        if (mealOption) {
            // Use ingredients directly since they already have proper measurements
            mealOption.ingredients.forEach(function (ingredient) {
                // Extract clean ingredient name from formatted text like "3 large free-range eggs" -> "eggs"
                var cleanIngredient = cleanIngredientName(ingredient);
                var existing = ingredientAmounts.get(cleanIngredient);
                if (existing) {
                    existing.count += 1;
                    // Add the default portion amount for each occurrence
                    var defaultPortion = getDefaultPortion(cleanIngredient);
    ...
    var shoppingList = [];
    ingredientAmounts.forEach(function (amounts, ingredient) {
        var finalIngredient = ingredient;
        ...
        var category = ingredientCategories[ingredient] || 'Other';
        // Convert amounts to grams using the formatAmount function
        var displayAmount = formatAmountWithLanguage(amounts.totalAmount, amounts.unit, language);
        // Determine final unit after conversion
        var finalUnit = amounts.unit;
        shoppingList.push({
    ...
    return shoppingList.sort(function (a, b) {
...
function parseEnhancedRecipeIngredients(instructions, ingredientAmounts) {
    instructions.forEach(function (instruction) {
        // Extract ingredients with amounts from recipe instructions
        var ingredientMatches = instruction.match(/(\d+(?:\.\d+)?)?\s*(cup|cups|tbsp|tsp|g|lb|lbs|oz|pieces?|cloves?|slices?)\s+([^,]+)/gi);
        if (ingredientMatches) {
            ingredientMatches.forEach(function (match) {
                var parts = match.trim().split(/\s+/);
                var amount = parseFloat(parts[0]) || 1;
                var unit = parts[1];
                var ingredient = parts.slice(2).join(' ').toLowerCase().trim();
                var existing = ingredientAmounts.get(ingredient);
                if (existing) {
                    existing.totalAmount += amount;
                    existing.count += 1;
                }
                else {
                    ingredientAmounts.set(ingredient, { totalAmount: amount, unit: unit, count: 1 });
                }
            });
        }
    });
...
function formatAmountWithLanguage(amount, unit, language) {
    if (language === void 0) { language = 'en'; }
    if (unit === 'ml') {
        if (amount >= 1000) {
            return "".concat((amount / 1000).toFixed(1), " L");
        }
        return "".concat(amount, " ml");
    }
    if (unit === 'g') {
        if (amount >= 1000) {
            return "".concat((amount / 1000).toFixed(1), " kg");
        }
        return "".concat(amount, " g");
    }
    if (unit === 'pieces' || unit === 'piece') {
        if (language === 'nl') {
            return amount === 1 ? "".concat(amount, " stuk") : "".concat(amount, " stuks");
        }
        return amount === 1 ? "".concat(amount, " piece") : "".concat(amount, " pieces");
    }
    if (unit === 'cloves' || unit === 'clove') {
        if (language === 'nl') {
            return amount === 1 ? "".concat(amount, " teen") : "".concat(amount, " tenen");
        }
        return amount === 1 ? "".concat(amount, " clove") : "".concat(amount, " cloves");
    }
...
function formatAmount(amount, unit) {
    ...
    if (finalAmount >= 1000) {
        return "".concat((finalAmount / 1000).toFixed(1), "kg");
    }
    else if (finalAmount < 1) {
        return "".concat((finalAmount * 1000).toFixed(0), "mg");
    }
    else {
        return "".concat(Math.round(finalAmount)).concat(finalUnit);
    }
}
